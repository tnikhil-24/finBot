# fetch_24_months_4h_extended.py
...
def get_last_n_months(n=24):  # Changed from 12 to 24
...
def fetch_24_months_4h_extended(symbol: str, save: bool = True, delay_seconds: int = 12) -> pd.DataFrame:
    months = get_last_n_months(24)
    all_data = []

    print(f"🔄 Fetching 1-hour data with extended hours for last 24 months for: {symbol}")
    for month in months:
    ...
    if save:
        filename = f"{symbol}_4h_24mo_extended.csv"
...
if __name__ == "__main__":
    symbol = "AAPL"
    df = fetch_24_months_4h_extended(symbol, save=True)
    print(df.tail())
